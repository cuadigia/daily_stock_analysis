...
"""
        if news_context:
            prompt += f"""
以下是 **{stock_name}({code})** 的多来源情报；短期事件窗口为近{news_window_days}日，研报、财报和行业资料最长回看 180 日。请重点提取：
1. 🚨 **风险警报**：减持、处罚、利空
2. 🎯 **利好催化**：业绩、合同、政策
3. 📊 **业绩预期**：年报预告、业绩快报
4. 🕒 **时间规则（强制）**：
   - 输出到 `risk_alerts` / `positive_catalysts` / `latest_news` 的每一条都必须带具体日期（YYYY-MM-DD）
   - 最新消息、风险事件和短期催化超出近{news_window_days}日窗口的一律忽略
   - 机构评级、财报和行业背景可使用最近 180 日资料，但必须标明日期，且不得表述为“今日/最新”
   - 时间未知、无法确定发布日期的内容不得作为最新事件或交易催化
5. 🔎 **来源核验（强制）**：
   - 优先级：公司投资者关系/SEC/交易所公告 > Reuters 等主流财经媒体 > 其他网站
   - 同一重要事实尽量由两个独立来源交叉印证；只有单一非官方来源时必须降低置信度
   - 来源之间冲突时采用官方原始披露，并在 `risk_warning` 或 `data_sources` 中说明冲突
   - 不得把搜索摘要中的推测、评论或目标价当作已经发生的事实
